...
if "s" in params:
    ...
    if not long_url:
        ...
        st.markdown("""
            <div style="text-align: center; margin-top: 15vh; padding: 20px;">
                ...
                <p style="color: #a0aec0; max-width: 500px; margin: 0 auto 30px auto; font-size: 1.1rem;">
                    The short link you are trying to access doesn't exist or has been deleted. Make sure you entered the correct address.
                </p>
            </div>
        """, unsafe_allow_html=True)
        ...
        # Valid short link - handle redirect & analytics collection
        if "logged" not in params:
            # First pass: Render geolocation and user-agent analyzer in the user's browser
            st.markdown(f"""
                <div style="text-align: center; margin-top: 25vh;">
                    <div style="display: inline-block; width: 60px; height: 60px; border: 4px rgba(255,255,255,0.1) solid; border-top: 4px #E94057 solid; border-radius: 50%; animation: spin 1s linear infinite; margin-bottom: 30px;"></div>
                    <h3 style="color: white; font-weight: 600; font-size: 1.5rem;">Redirecting...</h3>
                    <p style="color: #a0aec0; margin-top: 10px;">Analyzing connection and transferring you safely.</p>
                </div>
                <style>
                    @keyframes spin {{
            ...
            # Embed JS to gather user location (via ipapi.co) and browser information,
            # then reload the Streamlit app passing this metadata back in query parameters.
            st.components.v1.html(f"""
                <script>
                    async function collectAndRedirect() {{
                        ...
                        // 1. Fetch Geolocation details with timeout
                        try {{
                        ...
                        // 3. Build reload URL with collected analytics
                        const redirectUrl = new URL(window.location.href);
                        redirectUrl.searchParams.set("logged", "y");
            ...
            # Second pass: We have the client metadata in query parameters, log it and perform top-level redirect
            country = params.get("country", "Unknown")
            city = params.get("city", "Unknown")
            device = params.get("device", "Unknown")
            os = params.get("os", "Unknown")
            browser = params.get("browser", "Unknown")
            
            # Log the click in database
            db.log_click(
            ...
            # Show a friendly redirection card with a direct link fallback
            st.markdown(f"""
            ...
            # Perform final redirection to external site
            st.components.v1.html(f"""
                <script>
    ...
    # --- TAB 2: ANALYTICS DASHBOARD ---
        ...
        if links_df.empty:
            ...
            if not selected_code:
                ...
                # Single link metrics
                # Get the link click details from the df
            ...
            if analytics['total_clicks'] == 0:
                ...
                if not clicks_df.empty:
                    ...
                    # Choose aggregation window depending on total clicks or age
                    ...
                    with st.container():
                        ...
                        with c2:
                            if st.button("Delete", key=f"del_{row['short_code']}", use_container_width=True):
                                ...
                                st.rerun()
                        with c3:
                            # Let them view analytics directly
                            if st.button("Analytics", key=f"an_{row['short_code']}", use_container_width=True):
                                # Programmatically switch tab using session state (optional, or just prompt user)
